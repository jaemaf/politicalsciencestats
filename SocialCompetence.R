...
projEcon <- read.csv("./1_ProjektionEconSoc.txt", sep="")
...
lmer0 <- lmer(Zustimmung.SG ~ (1|d_diff) + (1|CDU.CSU) , data_long)
lmer1 <- lmer(Zustimmung.SG ~ Partei + (1|d_diff), data_long)
anova(lmer0, lmer1)
...
## first stats
fit.SPD <- lm(SPD ~ CDU.CSU.E + SPD.E + keine.E +  
		CDU.CSU.W + SPD.W + keine.W +  
		CDU.CSU.S + SPD.S + keine.S + d_diff, data = projEcon)
summary(fit.SPD)

fit.CDU.CSU <- lm(CDU.CSU ~ CDU.CSU.E + SPD.E + keine.E +  
		CDU.CSU.W + SPD.W + keine.W +  
		CDU.CSU.S + SPD.S + keine.S + d_diff, data = projEcon)
summary(fit.CDU.CSU)

fit.Linke <- lm(Linke ~ CDU.CSU.E + SPD.E + keine.E +  
		CDU.CSU.W + SPD.W + keine.W +  
		CDU.CSU.S + SPD.S + keine.S + d_diff, data = projEcon)
summary(fit.Linke)

fit.Grüne <- lm(Grüne ~ CDU.CSU.E + SPD.E + keine.E +  
		CDU.CSU.W + SPD.W + keine.W +  
		CDU.CSU.S + SPD.S + keine.S + d_diff, data = projEcon)
summary(fit.Grüne)

fit.FDP <- lm(FDP ~ CDU.CSU.E + SPD.E + keine.E +  
		CDU.CSU.W + SPD.W + keine.W +  
		CDU.CSU.S + SPD.S + keine.S + d_diff, data = projEcon)
summary(fit.FDP)

fit.AfD <- lm(AfD ~ CDU.CSU.E + SPD.E + keine.E +  
		CDU.CSU.W + SPD.W + keine.W +  
		CDU.CSU.S + SPD.S + keine.S + d_diff, data = projEcon)
summary(fit.AfD)
